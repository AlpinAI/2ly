...
# Priority 1: Use ENV variables if SYSTEM_KEY is provided
if [ -n "$SYSTEM_KEY" ]; then
  echo "✅ Using SYSTEM_KEY from environment variables"
  echo ""

# Priority 2: Load existing keys from shared directory
elif [ -f "$KEYS_DIR/.env.generated" ]; then
  echo "✅ Loading keys from $KEYS_DIR/.env.generated"
  echo ""

else
  echo "❌ ERROR: No SYSTEM_KEY available!"
  echo ""
  echo "Runtime requires SYSTEM_KEY to operate."
  echo ""
  echo "How to fix:"
  ...
  echo "  3. Production: Provide SYSTEM_KEY via environment variable"
  echo ""
