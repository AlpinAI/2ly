 ...
 *
 * WHY: App-wide conversational dialog for creating new tool sets (agents) with AI assistance.
 ...
 * 4. Tool set is created with metadata and selected tools in one step
 *
 * FEATURES:
 ...
 * - Creates runtime with ['agent'] capabilities and links suggested tools
 */
...
export function CreateToolSetDialog() {
  ...
  const [createRuntime, { loading: createLoading }] = useMutation(CreateRuntimeDocument);
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      const runtimeId = runtimeResult.data?.createRuntime.id;
      if (!runtimeId) throw new Error('Failed to create runtime');
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Failed to create tool set',
        variant: 'error',
      });
    }
  }, [name, description, workspaceId, selectedToolIds, createRuntime, linkTool, callback, close, toast]);
  ...
  return (
    <FormDialog
      ...
      title={step === 'goal' ? 'Create Tool Set with AI' : 'Review & Create Tool Set'}
      subtitle={
        step === 'goal'
      ...
      submitLabel={step === 'goal' ? 'Get AI Suggestions' : 'Create Tool Set'}
    ...
    >
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
              Describe your goal in natural language. AI will suggest a tool set name, description, and relevant tools.
            </p>
          </div>
      ...
      {step === 'review' && aiSuggestion && (
        <div className="space-y-4">
          {/* Name Field */}
          <div>
            <Label htmlFor="tool-set-name" className="flex items-center gap-2">
          ...
          {/* Description Field */}
          <div>
            <Label htmlFor="tool-set-description" className="flex items-center gap-2">
              Description
          ...
          {selectedToolIds.size > 0 && (
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              {selectedToolIds.size} tool{selectedToolIds.size !== 1 ? 's' : ''} will be added to this tool set
            </div>
          )}
        </div>
      )}
