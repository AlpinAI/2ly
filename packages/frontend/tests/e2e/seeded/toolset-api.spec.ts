...
test.describe('ToolSet API', () => {
  ...
  test('should verify dual-write pattern when linking tool to runtime', async ({ page, graphql }) => {
    // Login
    await performLogin(page, 'user1@example.com', 'password123');
    const workspaceUrl = page.url();
    const workspaceId = workspaceUrl.match(/\/w\/([^/]+)/)?.[1];
    ...
    const runtimeResult = await graphql<{
      createRuntime: { id: string; name: string; description: string; capabilities: string[] };
    }>(createRuntimeMutation, {
      name: 'Test Agent Runtime',
      description: 'For testing dual-write',
      capabilities: ['agent', 'tool'],
      workspaceId,
    });

    const runtimeId = runtimeResult.createRuntime.id;
    const runtimeName = runtimeResult.createRuntime.name;
    ...
    // If there are tools available, link one to the runtime
    if (toolsResult.mcpTools.length > 0) {
      const mcpToolId = toolsResult.mcpTools[0].id;

      // Link tool to runtime (old API)
      ...
      // Wait a bit for dual-write to complete
      await page.waitForTimeout(1000);

      // Verify a ToolSet was created with the runtime's name
      const queryToolSets = `
        query GetToolSets($workspaceId: ID!) {
          toolSets(workspaceId: $workspaceId) {
            id
      ...
      // Find the toolset created by dual-write (should have runtime's name)
      const dualWriteToolSet = toolsetsResult.toolSets.find((ts) => ts.name === runtimeName);

      expect(dualWriteToolSet).toBeDefined();
      expect(dualWriteToolSet!.mcpTools.length).toBeGreaterThan(0);
      expect(dualWriteToolSet!.mcpTools.some((tool) => tool.id === mcpToolId)).toBe(true);
    }
  });
