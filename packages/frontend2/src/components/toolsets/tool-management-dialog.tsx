/**
 * ToolManagementDialog Component
 *
 * WHY: Dialog modal for managing which tools belong to a tool set (runtime).
 * Provides search, filtering, and selection interface for linking/unlinking tools.
 *
 * FEATURES:
 * - Radix Dialog wrapper with proper z-index and animations
...
export function ToolManagementDialog() {
    ...
    return Array.from(serverMap.values())
      .filter(server => {
        ...
        if (searchTerm) {
          ...
          const serverMatches = server.name.toLowerCase().includes(term) || 
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] max-h-[90vh] w-[90vw] max-w-6xl translate-x-[-50%] translate-y-[-50%] rounded-lg border border-gray-200 bg-white p-0 shadow-lg data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] dark:border-gray-700 dark:bg-gray-800">
          <Dialog.Title className="sr-only">Manage Tools for {selectedToolSet.name}</Dialog.Title>
              ...
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={cn(
                      "font-medium",
                      hasChanges ? "text-orange-600 dark:text-orange-400" : "text-gray-500"
                    )}>
