---
title: "API REST"
description: "Deploy REST API integrations as tools in Skilder runtimes"
draft: true
---


REST API integrations provide HTTP endpoint access to external services through Skilder's tool runtime. APIs are wrapped with authentication, request/response mapping, and error handling to create standardized tool interfaces for agents.

## REST API Integration

Skilder transforms HTTP APIs into standardized tools that agents can invoke:
...
Configure REST APIs as tools through the Skilder interface:
...
Skilder supports multiple authentication patterns:
...
```json
{
  "response_mapping": {
    ...
    "computed_field": {
      "type": "template",
      "template": "Temperature is {{response.main.temp}}°C"
    }
  }
}
```
...
Skilder provides comprehensive API tool monitoring:
...
REST API tools in Skilder provide:
