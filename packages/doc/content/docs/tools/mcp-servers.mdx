---
title: "MCP Servers"
description: "Integrate any MCP server in Skilder"
draft: false
---

Skilder integrates any Model Context Protocol (MCP) server available on the market—from official provider implementations to community packages. Configure once in the Tool Registry, use across all agents.
...
All configured MCP servers are stored in Skilder's **Tool Registry**—a centralized catalog of available tools:
...
Skilder supports all MCP server communication patterns:
...
Skilder provides two ways to add MCP servers to your Tool Registry:

### 1. Browse Official Catalog

Access the official Anthropic MCP server catalog directly from Skilder:
...
Before deploying MCP servers, Skilder provides built-in testing:
