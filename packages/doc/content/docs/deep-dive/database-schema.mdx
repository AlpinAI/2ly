...
description: 'DGraph database schema and data models in Skilder'
draft: true
---

Skilder uses DGraph, a GraphQL-native graph database, for storing platform data. The schema defines workspaces, runtimes, MCP servers, tools, and their relationships.

## Core Entities

### System

The singleton representing the Skilder instance:
...
type MCPServer {
  ...
  runOn: MCPServerRunOn # AGENT | EDGE
...
1. **AGENT**: `MCPServer` spawns on agent's runtime (no DB link)
2. **EDGE**: `MCPServer` → specific `Runtime` (via `runtime` field)
...
# Add tool to a Skill
mutation {
  addMCPToolToSkill(mcpToolId: "...", skillId: "...") {
    id
    mcpTools {
      id
      name
