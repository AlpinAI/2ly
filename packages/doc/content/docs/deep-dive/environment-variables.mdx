...
description: "Complete reference for Skilder environment variables"
draft: true
---

Complete reference for configuring Skilder components via environment variables.

## Runtime Variables

Configure `@skilder-ai/runtime` behavior:
...
Configure `@skilder-ai/backend` API server:
...
Configure `@skilder-ai/frontend` React application:
...
```bash
VITE_API_URL=https://api.Skilder.ai \
VITE_WS_URL=wss://api.Skilder.ai \
...
VITE_API_URL=https://staging-api.Skilder.ai
```
...
metadata:
  name: Skilder-config
data:
  NATS_SERVERS: "nats://nats.Skilder.svc.cluster.local:4222"
  DGRAPH_URL: "dgraph.Skilder.svc.cluster.local:9080"
  LOG_LEVEL: "info"
---
apiVersion: apps/v1
kind: Deployment
