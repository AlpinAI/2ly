---
title: 'Tool Routing'
description: 'How tool execution requests are routed in Skilder'
draft: true
---

When an agent calls a tool, Skilder routes the execution request to the appropriate runtime based on the MCP server's `runOn` configuration. This page explains the complete routing flow.
...
Before an agent can call a tool, the tool must be organized into a Skill:

### Organizing Tools into Skills

```graphql
mutation {
  # Create a Skill
  createSkill(name: "My Tools", description: "Tools for my agent", workspaceId: "workspace-id") {
    id
  }

  # Add tool to Skill
  addMCPToolToSkill(mcpToolId: "tool-id", skillId: "skill-id")
}
```

This creates an edge: `Skill --[mcpTools]--> MCPTool`
...
### Example 1: Filesystem Tool (EDGE)
...
# Create server
mutation {
  createMCPServer(name: "Docker", command: "npx", args: "@modelcontextprotocol/server-docker", runOn: EDGE) {
    id
  }
}
...
mutation {
  linkMCPServerToRuntime(mcpServerId: "docker-server-id", runtimeId: "alice-edge-runtime-id")
}

# Create a Skill and add the tool
mutation {
  createSkill(name: "Alice's Tools", description: "Tools for Alice's agent", workspaceId: "workspace-id") {
    id
  }

  addMCPToolToSkill(mcpToolId: "docker-list-containers-id", skillId: "alice-skill-id")
}
```
...
```graphql
mutation {
  createMCPServer(name: "Session Context", command: "npx", args: "@example/session-context", runOn: AGENT)
}
```
