...
Before an agent can call a tool, the tool must be organized into a ToolSet:
...
mutation {
  # Create a ToolSet
  createToolSet(name: "My Tools", description: "Tools for my agent", workspaceId: "workspace-id") {
    id
  }

  # Add tool to ToolSet
  addMCPToolToToolSet(mcpToolId: "tool-id", toolSetId: "toolset-id")
}
```

This creates an edge: `ToolSet --[mcpTools]--> MCPTool`
...
# Create a ToolSet and add the tool
mutation {
  createToolSet(name: "Alice's Tools", description: "Tools for Alice's agent", workspaceId: "workspace-id") {
    id
  }

  addMCPToolToToolSet(mcpToolId: "docker-list-containers-id", toolSetId: "alice-toolset-id")
}
```
