...
Skilder implements a distributed, message-driven architecture for AI tool orchestration. The platform connects AI agents to MCP (Model Context Protocol) tools through distributed runtimes, using NATS messaging for communication and DGraph for persistence.

## Core Architecture

Skilder follows a layered architecture pattern with distributed components:
...
Python SDK for connecting AI agents to Skilder:
...
services:
  ...
  nats:
    ...
    command: ['--jetstream', '--cluster_name', 'Skilder-cluster', '--store_dir', '/data']
    volumes:
      - nats-data:/data
```
...
- Use appropriate runtime types (AGENT/EDGE) for use cases
