...
description: 'Deep dive into Skilder runtime architecture and services'
draft: true
---

The Skilder runtime is a distributed execution environment that can act as both an MCP server (agent mode) and MCP client (tool mode). It's published as `@skilder-ai/runtime` on npm.
...
server.setRequestHandler(InitializeRequestSchema, async () => {
  return {
    serverInfo: { name: "Skilder", version: "1.0.0" },
...
```typescript
const client = new Client({ name: 'Skilder', version: '1.0.0' });
...
NATS_SERVERS=nats://company-Skilder.com:4222 \
