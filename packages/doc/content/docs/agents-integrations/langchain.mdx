---
title: "LangChain"
description: "Connect LangChain agents to Skilder"
draft: false
---

LangChain agents connect to Skilder through the **MCPAdapter**, providing access to your configured tools via the MCP protocol.

## Overview

The **langchain_Skilder** connector enables LangChain agents to access Skilder's tool ecosystem. This integration:

- Connects LangChain/LangGraph agents to Skilder's MCP runtime
...
- Auto-creates agents in Skilder if they don't exist
...
The connector contains the MCPAdapter to quickly connect to Skilder.

### 2. Use Tools in LangGraph

```python
from langchain_Skilder import MCPAdapter
...
When instantiating the MCPAdapter, give it the name of your agent. Agents are automatically created in Skilder if they don't yet exist.
...
```bash
# Verify Skilder services are running
docker-compose ps
...
**Tools Not Loading**
- Verify agent name matches in Skilder dashboard
