...
Common issues and their solutions for Skilder deployments.
...
```bash
# Check runtime logs
npx @skilder-ai/runtime 2>&1 | tee runtime.log
...
# In dashboard: Tools → View configured tools
```
...
# Check tool logs
npx @skilder-ai/runtime --log-level=debug
```
...
LOG_LEVEL=debug npx @skilder-ai/runtime
...
- Ensure @skilder-ai/runtime installed
...
```bash
# Docker logs
docker logs -f Skilder-backend
docker logs -f Skilder-runtime
docker logs -f Skilder-nats
...
2. Review [GitHub Issues](https://github.com/skilder-ai/skilder/issues)
