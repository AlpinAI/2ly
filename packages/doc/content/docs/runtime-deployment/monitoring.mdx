...
description: 'Monitor Skilder platform health and performance'
draft: true
---

Effective monitoring ensures your Skilder deployment runs smoothly. This guide covers key metrics, health checks, and troubleshooting approaches.
...
subscription.subscribe({
  next: ({ data }) => {
    // Update dashboard
    const inactiveRuntimes = data.runtimes.filter((r) => r.status === 'INACTIVE');
...
```bash
# Docker logs
docker logs Skilder-backend --tail 100 -f

# Look for errors
docker logs Skilder-backend 2>&1 | grep ERROR
```
...
docker logs Skilder-main-runtime --tail 100 -f
...
Skilder does not currently export Prometheus metrics, but planned implementation:
...
1. Is runtime process still running?
   ...
   ```bash
   ps aux | grep @skilder-ai/runtime
   ```
...
3. Verify runOn configuration (AGENT vs EDGE)
