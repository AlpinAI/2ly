...
description: "Security considerations and best practices for Skilder deployment"
draft: true
---

This guide covers current security features and recommendations for securing your Skilder deployment. Note that authentication and authorization are minimal in the current version.
...
ROOTS=USER_HOME:/home/$USER/Skilder-workspace

# Run as user (not root)
su - $USER -c "npx @skilder-ai/runtime"
```
...
Deploy separate Skilder instances per tenant:
```bash
# Tenant A
...
```bash
# Failed connections
docker logs Skilder-backend | grep "connection refused"

# Unusual tool execution patterns
docker logs Skilder-backend | grep "AgentCallMCPTool"

# Runtime anomalies
docker logs Skilder-backend | grep "INACTIVE"
```
